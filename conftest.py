...
def compare_outputs(expected_outputs, actual_outputs):
    ...
    for i, (expected, actual) in enumerate(zip(expected_outputs, actual_outputs)):
        ...
        if expected['output_type'] != actual['output_type']:
            differences.append(f"Output {i} types differ: expected {expected['output_type']}, got {actual['output_type']}")
            continue
...
class PristineStateTest(pytest.Item):
        ...
        for i, cell in enumerate(self.notebook.cells):
            if cell.cell_type == 'code':
                ...
                if cell.execution_count is None:
                    ...
                    failures.append(f"Cell {i} has execution count {cell.execution_count}, expected {expected_exec_count}")
                expected_exec_count += 1
    ...
    def reportinfo(self):
        return self.fspath, 0, f"Checking notebook pristine state"
...
class NotebookExecutionTest(pytest.Item):
    ...
    def reportinfo(self):
        return self.fspath, 0, f"Testing notebook execution"
