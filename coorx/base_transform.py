...
class BaseTransform(object):
    ...
    def __getstate__(self):
        ...
        state['systems'] = [None if sys is None else sys.name for sys in self.systems]
        state['_cs_graph'] = None if self._cs_graph is None else self._cs_graph.name
        return state

    def __setstate__(self, state):
        systems = state.pop('systems', (None, None))
        graph = state.pop('_cs_graph', None)
        self.__dict__.update(state)        
        self.set_systems(systems[0], systems[1], graph)

    def __eq__(self, tr):
        return self.params == tr.params
